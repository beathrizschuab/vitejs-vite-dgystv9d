...
const APPT_STATUS_LIST=["Confirmado","Aguardando","Realizado","Cancelado","Faltou","Reagendado"];
...
const newId = () => String(Date.now()) + String(Math.random()).slice(2,7);
...
function useSupaTable(table, initFallback=[]) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      uid.current = user.id;
      supabase.from(table).select("*").eq("user_id", user.id).then(({ data: rows, error }) => {
        if (!cancelled) {
          if (rows && rows.length > 0) setDataRaw(rows);
          else setDataRaw(initFallback);
          setLoading(false);
        }
      });
    });
  ...
  const setData = useCallback((valOrFn) => {
    setDataRaw(prev => {
      ...
      if (uid.current) {
        const toUpsert = safeList(next).map(r => ({
          ...r,
          id: r.id || newId(),
          user_id: uid.current
        }));
        supabase.from(table).upsert(toUpsert, { onConflict: "id" }).then(({ error }) => {
...
function LoginScreen({ onLogin }) {
  ...
  return h(Fragment, null,
    ...
    h("div",{style:{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:`radial-gradient(ellipse at 50% 0%, rgba(92,31,50,.35) 0%, ${P.bg} 65%)`}},
      h("div",{style:{width:380,padding:"48px 40px",background:P.bg2,border:`1px solid ${P.border}`,borderRadius:20,boxShadow:"0 32px 80px rgba(0,0,0,.6)"}},
        ...
        h("div",{style:{marginBottom:16}},
          ...
          h("input",{type:"email",value:email,onChange:e=>setEmail(e.target.value),onKeyDown:e=>e.key==="Enter"&&handleLogin(),placeholder:"seu@email.com",style:{width:"100%",background:P.bg3,border:`1px solid ${P.border}`,borderRadius:8,padding:"11px 14px",color:P.text,fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}})
        ),
        h("div",{style:{marginBottom:24}},
          ...
          h("input",{type:"password",value:password,onChange:e=>setPassword(e.target.value),onKeyDown:e=>e.key==="Enter"&&handleLogin(),placeholder:"••••••••",style:{width:"100%",background:P.bg3,border:`1px solid ${P.border}`,borderRadius:8,padding:"11px 14px",color:P.text,fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}})
        ),
        ...
        h("button",{onClick:handleLogin,disabled:loading,style:{width:"100%",padding:"13px",background:`linear-gradient(135deg,${P.rose},${P.gold})`,color:P.accent3,border:"none",borderRadius:8,fontSize:14,fontWeight:600,cursor:loading?"not-allowed":"pointer",fontFamily:"'DM Sans',sans-serif",opacity:loading?.7:1,letterSpacing:".04em"}},loading?"Entrando...":"Entrar"),
...
function Inp({value,onChange,placeholder,type="text",style:s}){return createElement("input",{value:value||"",onChange:e=>onChange(e.target.value),placeholder,type,style:{...IS,...s}});}
...
function TA({value,onChange,placeholder,rows=3}){return createElement("textarea",{value:value||"",onChange:e=>onChange(e.target.value),placeholder,rows,style:{...IS,resize:"vertical"}});}
function Btn({children,onClick,variant="primary",style:s,disabled=false}){
  const vs={
    primary:{background:`linear-gradient(135deg,${P.rose},${P.gold})`,color:P.accent3,border:"none"},
    ghost:{background:"transparent",color:P.text2,border:`1px solid ${P.border}`},
    danger:{background:"rgba(192,112,112,.1)",color:P.red,border:"1px solid rgba(192,112,112,.2)"},
    sm:{background:`linear-gradient(135deg,${P.rose},${P.gold})`,color:P.accent3,border:"none",padding:"5px 10px",fontSize:11}
  };
...
function SectionHeader({title,sub,action}){
  return createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}},
    createElement("div",null,
      createElement("div",{style:{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:P.text,letterSpacing:".02em",lineHeight:1.1}},title),
      sub&&createElement("div",{style:{fontSize:13,color:P.text3,marginTop:5}},sub)
    ),action);
}
function StatusBadge({status,cfg=PAT_STATUS_CFG}){
  const c=cfg[status]||{color:P.text3,bg:P.bg3,label:status||"—"};
  return createElement("span",{style:{display:"inline-block",padding:"3px 9px",borderRadius:20,fontSize:11,fontWeight:500,color:c.color,background:c.bg}},c.label);
}
function AlertBadge({text,color=P.red}){
  return createElement("span",{style:{display:"inline-block",padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600,color,background:color+"18",border:`1px solid ${color}44`,marginRight:6,marginBottom:4}},`⚠ ${text}`);
}
...
function Dashboard({patients,agenda,onNav,onSelectPatient,settings,returnRules}){
  ...
  const rets=useMemo(()=>{
    ...
    for(const p of patients||[]){
      const sessions=safeList(p.sessions);
      if(sessions.length===0) continue;
      const sorted=[...sessions].sort((a,b)=>parseDMY(b.date)-parseDMY(a.date));
      const last=sorted[0];
    ...
    h("div",{style:{display:"flex",gap:16,flexWrap:"wrap"}},
      h("div",{style:{flex:"1 1 calc(60% - 8px)",minWidth:320}},h(Card,null,
        h("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:16}},
          h("div",{style:{fontSize:14,fontWeight:600,color:P.accent3}},"Agenda de Hoje"),
          h(Btn,{variant:"ghost",onClick:()=>onNav("agenda"),style:{padding:"3px 10px",fontSize:11}},"Ver agenda")
        ),
        tAct.length===0
          ? h("div",{style:{color:P.text3,fontSize:13,padding:"20px 0",textAlign:"center"}},"Nenhum atendimento agendado para hoje.")
          : h("div",{style:{display:"flex",flexDirection:"column",gap:10}},tAct.map((a,i)=>{
              const pat=(patients||[]).find(p=>p.name===a.patientName);
              return h("div",{key:a.id||i,onClick:()=>{if(pat)onSelectPatient(pat);},style:{display:"flex",alignItems:"center",gap:12,padding:10,background:P.bg3,borderRadius:8,cursor:pat?"pointer":"default",border:`1px solid ${P.border}`}},
      ...
      h("div",{style:{flex:"1 1 calc(40% - 8px)",minWidth:260}},h(Card,null,
        ...
        rets.length===0
          ? h("div",{style:{color:P.text3,fontSize:13,padding:"20px 0"}},"Nenhum alerta de retorno no momento.")
          : h("div",{style:{display:"flex",flexDirection:"column",gap:10}},rets.map((r,i)=>
              h("div",{key:i,onClick:()=>onSelectPatient(r.patient),style:{padding:10,background:P.bg3,border:`1px solid ${P.border}`,borderRadius:8,cursor:"pointer"}},
...
function Sidebar({page,onNav,patients,onSelectPatient}){
  ...
  function select(p){onSelectPatient(p);setQ("");setOpenSearch(false);}
  const menu=[
    {k:"dashboard",l:"Painel",e:"✦"},{k:"agenda",l:"Agenda",e:"📅"},
    {k:"pacientes",l:"Pacientes",e:"👥"},{k:"prontuario",l:"Prontuário",e:"📝"},
    {k:"retornos",l:"Retornos",e:"⏳"},{k:"estoque",l:"Estoque",e:"📦"},
    {k:"financeiro",l:"Financeiro",e:"💰"},{k:"relatorios",l:"Relatórios",e:"📊"},
    {k:"mensagens",l:"Mensagens WhatsApp",e:"💬"},{k:"config",l:"Configurações",e:"⚙"}
  ];
  return h("div",{style:{width:240,background:P.bg2,borderRight:`1px solid ${P.border}`,display:"flex",flexDirection:"column",height:"100vh",flexShrink:0}},
    h("div",{style:{padding:"24px 20px",borderBottom:`1px solid ${P.border}`}},
      h("button",{onClick:()=>setOpenSearch(true),style:{width:"100%",background:P.bg3,border:`1px solid ${P.border}`,borderRadius:8,padding:"8px 12px",color:P.text3,fontSize:12.5,textAlign:"left",cursor:"pointer",display:"flex",alignItems:"center",gap:6}},
        h("span",null,"🔍"),"Buscar paciente..."
      ),
      h(Modal,{open:openSearch,onClose:()=>setOpenSearch(false),title:"Buscar Paciente",width:420},
        h(Inp,{value:q,onChange:setQ,placeholder:"Digite o nome..."}),
        h("div",{style:{marginTop:14,display:"flex",flexDirection:"column",gap:6,maxHeight:300,overflowY:"auto"}},
          ...
          filtered.map((p,i)=>h("div",{key:p.id||i,onClick:()=>select(p),style:{padding:"9px 12px",background:P.bg3,borderRadius:8,cursor:"pointer",fontSize:13.5,display:"flex",alignItems:"center",gap:10,border:`1px solid ${P.border}`}},
            h(Avatar,{name:p.name,size:24,idx:i}),p.name))
        )
      )
    ),
    h("div",{style:{flex:1,padding:"16px 12px",display:"flex",flexDirection:"column",gap:4,overflowY:"auto"}},
      menu.map(m=>h("button",{key:m.k,onClick:()=>onNav(m.k),style:{display:"flex",alignItems:"center",gap:12,width:"100%",padding:"10px 14px",background:page===m.k?P.rose:"transparent",color:page===m.k?P.accent3:P.text2,border:"none",borderRadius:8,fontSize:13,fontWeight:page===m.k?600:400,cursor:"pointer",textAlign:"left",transition:"all .15s"}},
        h("span",{style:{fontSize:14,opacity:page===m.k?1:.6}},m.e),m.l))
    ),
    h("div",{style:{padding:16,borderTop:`1px solid ${P.border}`,textAlign:"center"}},
      h(Btn,{variant:"ghost",onClick:async()=>{await supabase.auth.signOut();window.location.reload();},style:{width:"100%",fontSize:12}},"Sair da Conta"))
  );
}

// ─── AGENDA ──────────────────────────────────────────────────────────────────
function Agenda({agenda,setAgenda,procedures,locations}){
  const h=createElement;
  const emptyEvt={patientName:"",date:todayISO(),time:"09:00",procedure:procedures[0]||"",location:locations[0]||"",duration:"1 hora",value:"",status:"Confirmado",obs:""};
  const [modal,setModal]=useState(false);
  const [evt,setEvt]=useState(emptyEvt);
  function save(){
    if(!evt.patientName) return;
    const item={...evt,id:evt.id||newId(),value:Number(evt.value)||0};
    setAgenda(prev=>evt.id?safeList(prev).map(a=>a.id===evt.id?item:a):[...safeList(prev),item]);
    setModal(false);
  }
  function del(id){
    if(window.confirm("Excluir agendamento?")){
      setAgenda(prev=>safeList(prev).filter(a=>a.id!==id));
      setModal(false);
    }
  }
  const sorted=[...safeList(agenda)].sort((a,b)=>a.date.localeCompare(b.date)||(a.time||"").localeCompare(b.time||""));
  return h("div",null,
    h(SectionHeader,{title:"Agenda de Atendimentos",sub:"Gerencie seus horários e consultas",
      action:h(Btn,{onClick:()=>{setEvt(emptyEvt);setModal(true);}},"✦ Novo Agendamento")}),
    h(Card,{style:{padding:0,overflow:"hidden"}},
      h("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:13.5,textAlign:"left"}},
        h("thead",{style:{background:P.bg2,color:P.text3,fontSize:11,textTransform:"uppercase"}},
          h("tr",null,h("th",{style:{padding:"12px 16px"}},"Data/Hora"),h("th",null,"Paciente"),h("th",null,"Procedimento"),h("th",null,"Local"),h("th",null,"Valor"),h("th",null,"Status"),h("th",{style:{padding:"12px 16px",textAlign:"right"}},"Ações"))
        ),
...
// ─── PATIENTS LIST ────────────────────────────────────────────────────────────
function Patients({patients,setPatients,onSelect}){
  const h=createElement;
  const [modal,setModal]=useState(false);
  const [pat,setPat]=useState({name:"",age:"",phone:"",email:"",status:"new"});
  function save(){
    if(!pat.name) return;
    const item={...pat,id:pat.id||newId(),since:pat.since||new Date().toLocaleDateString("pt-BR"),sessions:[],anamnese:{}};
    setPatients(prev=>pat.id?safeList(prev).map(p=>p.id===pat.id?item:p):[...safeList(prev),item]);
    setModal(false);
  }
  return h("div",null,
    h(SectionHeader,{title:"Carteira de Pacientes",sub:"Históricos, fichas e registros",
      action:h(Btn,{onClick:()=>{setPat({name:"",age:"",phone:"",email:"",status:"new"});setModal(true);}},"✦ Cadastrar Paciente")}),
    h("div",{style:{display:"flex",flexDirection:"column",gap:10}},
      safeList(patients).map((p,i)=>h(Card,{key:p.id||i,onClick:()=>onSelect(p),style:{display:"flex",alignItems:"center",gap:16,padding:16}},
        h(Avatar,{name:p.name,size:44,idx:i}),
        h("div",{style:{flex:1}},
          h("div",{style:{fontSize:15,fontWeight:600,color:P.accent3}},p.name),
          h("div",{style:{fontSize:12,color:P.text3,marginTop:3}},`Idade: ${p.age||"N/I"} · Tel: ${p.phone||"N/I"} · Desde: ${p.since||""}`)
        ),
    ...
    h(Modal,{open:modal,onClose:()=>setModal(false),title:"Ficha Cadastral"},
      h("div",{style:{display:"flex",flexWrap:"wrap",gap:12}},
        ...
        h(Field,{label:"E-mail",third:true},h(Inp,{placeholder:"email@email.com",value:pat.email,onChange:v=>setPat({...pat,email:v})})),
        h(Field,{label:"Status",half:true},h(Sel,{value:pat.status,onChange:v=>setPat({...pat,status:v}),options:Object.keys(PAT_STATUS_CFG)}))
      ),
      h("div",{style:{display:"flex",gap:8,justifyContent:"flex-end",marginTop:18}},
        h(Btn,{variant:"ghost",onClick:()=>setModal(false)},"Cancelar"),
        h(Btn,{onClick:save},"Salvar Paciente")
      )
    )
  );
}
...
function PatientDetail({patient,setPatients,onBack,procedures,locations,products,returnRules}){
  ...
  const emptySession={date:todayISO(),procedure:procedures[0]||"",doctor:"",product:products[0]||"",dose:"",region:"",location:locations[0]||"",value:"",finStatus:"Pago",payMethod:"Pix",notes:"",evolution:"",faceMap:null};
  const [sess,setSess]=useState(emptySession);

  // CORREÇÃO CRÍTICA: sempre lê as sessões direto do patient prop (que vem atualizado do state pai)
  const sessions = safeList(patient.sessions);

  function saveSess(){
    const nSess={...sess,id:sess.id||newId(),value:Number(sess.value)||0};
    const updSess=sess.id?sessions.map(s=>s.id===sess.id?nSess:s):[...sessions,nSess];
    setPatients(prev=>safeList(prev).map(p=>p.id===patient.id?{...p,sessions:updSess,lastVisit:new Date().toLocaleDateString("pt-BR")}:p));
    setSMod(false);
  }
  ...
  const alerts=useMemo(()=>{
    ...
    if(patient.anamnese?.importantAlerts) arr.push(...safeList(patient.anamnese.importantAlerts));
    return arr;
  },[patient]);
  ...
  return h("div",null,
    h("div",{style:{display:"flex",alignItems:"center",gap:12,marginBottom:20}},
      h(Btn,{variant:"ghost",onClick:onBack,style:{padding:"6px 12px"}},"← Voltar"),
      h("div",{style:{fontSize:13,color:P.text3}},`Ficha clínica / ${patient.name}`)
    ),
    h("div",{style:{display:"flex",gap:20,alignItems:"center",marginBottom:24,background:P.card,padding:20,borderRadius:14,border:`1px solid ${P.border}`}},
      ...
      h("div",{style:{flex:1}},
        h("h2",{style:{fontFamily:"serif",color:P.accent3,fontSize:22}},patient.name),
        h("div",{style:{fontSize:12.5,color:P.text3,marginTop:4}},`CPF: ${patient.cpf||"Não informado"} · Tel: ${patient.phone||""} · E-mail: ${patient.email||""}`)
      ),
    ...
    tab==="historico"&&h("div",null,
      h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
        h("div",{style:{fontSize:15,fontWeight:600,color:P.accent3}},"Aplicações Realizadas"),
        h(Btn,{onClick:()=>{setSess(emptySession);setSMod(true);},variant:"sm"},"+ Registrar Atendimento")
      ),
      sessions.length===0
        ? h("div",{style:{color:P.text3,textAlign:"center",padding:40}},"Nenhuma sessão clínica gravada.")
        : h("div",{style:{display:"flex",flexDirection:"column",gap:14}},sessions.map((s,i)=>h(Card,{key:s.id||i},
            h("div",{style:{display:"flex",justifyContent:"space-between"}},
              h("div",{style:{fontWeight:600,color:P.accent2,fontSize:14.5}},s.procedure),
              h("div",{style:{fontSize:12,color:P.text3}},s.date)
            ),
    ...
    tab==="anamnese"&&h(Card,null,
      h("div",{style:{display:"flex",flexWrap:"wrap",gap:14,fontSize:13.5}},
        ...
        h("div",{style:{flex:"1 1 calc(50% - 8px)"}},h("b",null,"Medicamentos: "),patient.anamnese?.medications||"Nenhum"),
        ...
        h("div",{style:{flex:"1 1 100%",borderTop:`1px solid ${P.border}`,paddingTop:12,marginTop:6}},h("b",null,"Procedimentos anteriores: "),patient.anamnese?.previousProcedures||"Nenhum")
      )
    ),
    ...
    h(Modal,{open:sMod,onClose:()=>setSMod(false),title:"Novo Registro Clínico",width:680},
      h("div",{style:{display:"flex",gap:20,flexWrap:"wrap"}},
        h("div",{style:{flex:"1 1 300px",display:"flex",flexDirection:"column",gap:10}},
          ...
          h("div",{style:{display:"flex",gap:10}},
            ...
            h(Field,{label:"Valor Praticado (R$)",half:true},h(Inp,{type:"number",placeholder:"0.00",value:sess.value,onChange:v=>setSess({...sess,value:v})}))
          ),
          h(Field,{label:"Região"},h(Inp,{placeholder:"Ex: Glabela, Lábio",value:sess.region||"",onChange:v=>setSess({...sess,region:v})})),
      ...
      h("div",{style:{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}},
        h(Btn,{variant:"ghost",onClick:()=>setSMod(false)},"Cancelar"),
        h(Btn,{onClick:saveSess},"Gravar Sessão")
      )
    )
  );
}
...
function Estoque({products,setProducts}){
  ...
  const [p,setP]=useState({name:"",qty:"",min:"",unit:"Unidades"});
  function save(){
    if(!p.name) return;
    const item={...p,id:p.id||newId()};
    setProducts(prev=>p.id?safeList(prev).map(x=>x.id===p.id?item:x):[...safeList(prev),item]);
    setM(false);
  }
  return h("div",null,
    h(SectionHeader,{title:"Controle de Insumos e Estoque",sub:"Evite quebras de materiais em tratamentos",
      action:h(Btn,{onClick:()=>{setP({name:"",qty:"",min:"",unit:"Unidades"});setM(true);}},"✦ Adicionar Material")}),
    h(Card,{style:{padding:0,overflow:"hidden"}},
      h("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:13.5,textAlign:"left"}},
        h("thead",{style:{background:P.bg2,color:P.text3,fontSize:11,textTransform:"uppercase"}},
          h("tr",null,h("th",{style:{padding:"12px 16px"}},"Item"),h("th",null,"Quantidade"),h("th",null,"Mínimo"),h("th",null,"Status"),h("th",{style:{padding:"12px 16px",textAlign:"right"}},"Ações"))
        ),
        h("tbody",null,safeList(products).map((x,i)=>{
          const q=Number(x.qty)||0,mn=Number(x.min)||0,low=q<=mn;
          return h("tr",{key:x.id||i,style:{borderBottom:`1px solid ${P.border}`}},
            h("td",{style:{padding:"14px 16px",fontWeight:500}},x.name),
            h("td",null,`${x.qty} ${x.unit||"un"}`),
            h("td",null,`${x.min} ${x.unit||"un"}`),
            h("td",null,h("span",{style:{color:low?P.red:P.green,fontWeight:500}},low?"⚠ Crítico":"✓ Seguro")),
            h("td",{style:{padding:"14px 16px",textAlign:"right"}},h(Btn,{variant:"ghost",onClick:()=>{setP(x);setM(true);},style:{padding:"4px 10px",fontSize:11}},"Editar"))
          );
        }))
      )
    ),
    h(Modal,{open:m,onClose:()=>setM(false),title:"Ficha de Produto"},
      h("div",{style:{display:"flex",flexWrap:"wrap",gap:12}},
        ...
        h(Field,{label:"Quantidade Atual",third:true},h(Inp,{type:"number",value:p.qty,onChange:v=>setP({...p,qty:v})})),
        h(Field,{label:"Alerta Mínimo",third:true},h(Inp,{type:"number",value:p.min,onChange:v=>setP({...p,min:v})})),
        h(Field,{label:"Unidade",third:true},h(Inp,{value:p.unit,onChange:v=>setP({...p,unit:v})}))
      ),
      h("div",{style:{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}},
        h(Btn,{variant:"ghost",onClick:()=>setM(false)},"Cancelar"),
        h(Btn,{onClick:save},"Salvar")
      )
    )
  );
}
...
function Financeiro({patients,expenses,setExpenses}){
  const h=createElement;
  const [m,setM]=useState(false);
  const [exp,setExp]=useState({desc:"",date:todayISO(),cat:EXPENSE_CATS[0],value:"",notes:""});

  const totInc=useMemo(()=>{
    let sum=0;
    for(const p of safeList(patients)){
      for(const s of safeList(p.sessions)){
        if(s.finStatus==="Pago") sum+=Number(s.value)||0;
      }
    }
    return sum;
  },[patients]);

  const totExp=useMemo(()=>safeList(expenses).reduce((acc,e)=>acc+Number(e.value||0),0),[expenses]);

  function saveExpense(){
    if(!exp.desc||!exp.value){ alert("Preencha descrição e valor."); return; }
    const item={...exp,id:newId(),value:Number(exp.value)};
    setExpenses(prev=>[...safeList(prev),item]);
    setExp({desc:"",date:todayISO(),cat:EXPENSE_CATS[0],value:"",notes:""});
    setM(false);
  }

  return h("div",null,
    h(SectionHeader,{title:"Fluxo Financeiro",sub:"Acompanhamento de caixa e notas",
      action:h(Btn,{onClick:()=>setM(true)},"✦ Lançar Despesa")}),
    h("div",{style:{display:"flex",gap:16,marginBottom:24,flexWrap:"wrap"}},
      h("div",{style:{flex:1,minWidth:160}},h(Card,null,h("div",{style:{fontSize:11,textTransform:"uppercase",color:P.text3}},"Receitas (Pago)"),h("div",{style:{fontSize:26,fontFamily:"serif",color:P.green,marginTop:4}},fmtCurr(totInc)))),
      h("div",{style:{flex:1,minWidth:160}},h(Card,null,h("div",{style:{fontSize:11,textTransform:"uppercase",color:P.text3}},"Despesas"),h("div",{style:{fontSize:26,fontFamily:"serif",color:P.red,marginTop:4}},fmtCurr(totExp)))),
      h("div",{style:{flex:1,minWidth:160}},h(Card,null,h("div",{style:{fontSize:11,textTransform:"uppercase",color:P.text3}},"Balanço Líquido"),h("div",{style:{fontSize:26,fontFamily:"serif",color:totInc-totExp>=0?P.green:P.red,marginTop:4}},fmtCurr(totInc-totExp))))
    ),
    h(Card,{style:{padding:0,overflow:"hidden"}},
      h("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:13.5,textAlign:"left"}},
        h("thead",{style:{background:P.bg2,color:P.text3,fontSize:11,textTransform:"uppercase"}},
          h("tr",null,h("th",{style:{padding:"12px 16px"}},"Descrição"),h("th",null,"Data"),h("th",null,"Categoria"),h("th",null,"Valor"),h("th",{style:{padding:"12px 16px",textAlign:"right"}},"Tipo"))
        ),
        h("tbody",null,safeList(expenses).map((e,i)=>h("tr",{key:e.id||i,style:{borderBottom:`1px solid ${P.border}`}},
          h("td",{style:{padding:"14px 16px",fontWeight:500}},e.desc),
          h("td",null,e.date),h("td",null,e.cat),h("td",null,fmtCurr(e.value)),
    ...
    h(Modal,{open:m,onClose:()=>setM(false),title:"Nova Despesa"},
      h("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        h(Field,{label:"Descrição"},h(Inp,{value:exp.desc,onChange:v=>setExp({...exp,desc:v}),placeholder:"Ex: Aluguel, Material"})),
        h("div",{style:{display:"flex",gap:10}},
          h(Field,{label:"Valor (R$)",half:true},h(Inp,{type:"number",value:exp.value,onChange:v=>setExp({...exp,value:v}),placeholder:"0.00"})),
          h(Field,{label:"Data",half:true},h(Inp,{type:"date",value:exp.date,onChange:v=>setExp({...exp,date:v})}))
        ),
        h(Field,{label:"Categoria"},h(Sel,{value:exp.cat,onChange:v=>setExp({...exp,cat:v}),options:EXPENSE_CATS})),
        h(Field,{label:"Notas (opcional)"},h(TA,{value:exp.notes,onChange:v=>setExp({...exp,notes:v}),rows:2,placeholder:"Observações..."}))
      ),
      h("div",{style:{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}},
        h(Btn,{variant:"ghost",onClick:()=>setM(false)},"Cancelar"),
        h(Btn,{onClick:saveExpense},"Lançar Despesa")
      )
    )
  );
}
...
function Relatorios({patients}){
  ...
  const calculated=useMemo(()=>{
    ...
    for(const p of safeList(patients)){
      for(const s of safeList(p.sessions)){
        tot+=Number(s.value)||0;
        if(s.procedure) procs[s.procedure]=(procs[s.procedure]||0)+1;
      }
    }
    return {tot,procs:Object.entries(procs).sort((a,b)=>b[1]-a[1])};
  },[patients]);
  ...
  return h("div",null,
    ...
    h("div",{style:{display:"flex",gap:16,flexWrap:"wrap"}},
      h("div",{style:{flex:"1 1 300px"}},h(Card,null,
        h("div",{style:{fontSize:14,fontWeight:600,color:P.accent3,marginBottom:16}},"Ranking de Procedimentos"),
        calculated.procs.length===0
          ? h("div",{style:{color:P.text3,fontSize:13}},"Nenhum dado clínico acumulado.")
          : h("div",{style:{display:"flex",flexDirection:"column",gap:12}},calculated.procs.map(([name,count],i)=>h("div",{key:i},
              ...
              h("div",{style:{width:"100%",background:P.bg3,height:6,borderRadius:3,overflow:"hidden"}},h("div",{style:{background:P.rose,height:"100%",width:`${Math.min(count*15,100)}%`}}))
            )))
      )),
      h("div",{style:{flex:"1 1 240px"}},h(Card,null,
        h("div",{style:{fontSize:13,fontWeight:600,color:P.accent3}},"Volume Total Faturado"),
        h("div",{style:{fontSize:32,fontFamily:"serif",color:P.green,marginTop:10}},fmtCurr(calculated.tot)),
        h("p",{style:{fontSize:12,color:P.text3,marginTop:6}},"Receita bruta das evoluções gravadas.")
      ))
    )
  );
}
...
function MensagensWhatsApp(){
  const h=createElement;
  return h("div",null,
    h(SectionHeader,{title:"Mensagens e Alertas WhatsApp",sub:"Modelos de textos para pós-procedimento"}),
    h("div",{style:{display:"flex",flexDirection:"column",gap:14}},
      h(Card,null,
        ...
        h("div",{style:{background:P.bg3,padding:14,borderRadius:8,fontSize:13,color:P.text2,fontFamily:"monospace",whiteSpace:"pre-wrap"}},
          "Olá {nome}, tudo bem? Aqui é da clínica HarmonizaPro.\n\nLembrando que já completaram 14 dias desde a sua aplicação de Toxina Botulínica. Esse é o momento ideal para avaliarmos o resultado e realizar qualquer retoque necessário.\n\nPodemos confirmar o seu horário para essa semana?"
        )
      ),
      h(Card,null,
        h("div",{style:{fontWeight:600,color:P.accent3,fontSize:14.5,marginBottom:8}},"Template 2: Manutenção Pós Preenchimento (6 meses)"),
        h("div",{style:{background:P.bg3,padding:14,borderRadius:8,fontSize:13,color:P.text2,fontFamily:"monospace",whiteSpace:"pre-wrap"}},
          "Olá {nome}! Esperamos que esteja amando o resultado.\n\nFaz cerca de 6 meses desde o seu último preenchimento. Esse é o período indicado para uma nova sessão de manutenção.\n\nVamos agendar um horário?"
        )
      )
    )
  );
...
// ─── CONFIGURAÇÕES ────────────────────────────────────────────────────────────
function Configuracoes({settings,setSettings,products,setProducts}){
  const h=createElement;
  ...
  function saveProfile(){
    setSettings({...settings,doctorName:docName,doctorTitle:docTitle,clinicName:clName});
    alert("Perfil gravado com sucesso!");
  }
  return h("div",null,
    h(SectionHeader,{title:"Configurações Globais",sub:"Ajuste os parâmetros do seu consultório"}),
    h(Card,null,
      h("div",{style:{fontSize:15,fontWeight:600,color:P.accent3,marginBottom:16}},"Perfil Profissional"),
      ...
      h("div",{style:{textAlign:"right"}},h(Btn,{onClick:saveProfile},"Gravar Alterações"))
    )
  );
}

// ─── RETORNOS PENDENTES ───────────────────────────────────────────────────────
function RetornosPendentes({patients,returnRules,onSelectPatient}){
  const h=createElement;
  const rets=useMemo(()=>{
    const list=[];
    const rulesMap=new Map(safeList(returnRules).map(r=>[r.procedure,r]));
    for(const p of safeList(patients)){
      const sessions=safeList(p.sessions);
      if(sessions.length===0) continue;
      const sorted=[...sessions].sort((a,b)=>parseDMY(b.date)-parseDMY(a.date));
  ...
  return h("div",null,
    ...
    rets.length===0
      ? h(Card,null,h("div",{style:{color:P.text3,textAlign:"center",padding:40}},"Nenhum retorno pendente."))
      : h("div",{style:{display:"flex",flexDirection:"column",gap:10}},rets.map((r,i)=>
          h(Card,{key:i,onClick:()=>onSelectPatient(r.patient),style:{display:"flex",alignItems:"center",gap:16,padding:16}},
...
export default function App() {
  ...
  // CORREÇÃO: guarda só o ID do paciente selecionado, não o objeto inteiro
  ...
  const [products, setProducts, prLoad] = useSupaTable("products",
    INIT_PRODUCTS.map((p,i)=>({id:String(i+1),name:p,qty:10,min:2,unit:"Unidades"})));
  const [expenses, setExpenses, eLoad] = useSupaTable("expenses", INIT_EXPENSES);
  const [returnRules] = useState(INIT_RETURN_RULES);
  ...
  // CORREÇÃO: busca o paciente sempre atualizado da lista
  const currentPatient = useMemo(()=>
    currentPatientId ? safeList(patients).find(p=>p.id===currentPatientId)||null : null
  ...
  const handleNav = useCallback((p) => {
    setPage(p);
    if (p !== "prontuario") setCurrentPatientId(null);
  }, []);
  ...
  if (authLoading) return h("div",{style:{minHeight:"100vh",background:P.bg,display:"flex",alignItems:"center",justifyContent:"center",color:P.text3,fontSize:14}},"Autenticando...");
  if (!session) return h(LoginScreen,{onLogin:()=>supabase.auth.getSession().then(({data:{session:s}})=>setSession(s))});
  ...
  return h("div",{style:{display:"flex",background:P.bg,color:P.text,minHeight:"100vh",fontFamily:"'DM Sans',sans-serif"}},
    ...
    `),
    h(Sidebar,{page,onNav:handleNav,patients,onSelectPatient:handleSelectPatient}),
    h("div",{style:{flex:1,display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden"}},
      appLoading&&h("div",{style:{padding:"8px 24px",background:"rgba(196,169,106,.1)",fontSize:12,color:P.yellow,textAlign:"center"}},"Sincronizando dados..."),
      h("div",{style:{flex:1,overflowY:"auto",padding:24}},
        ...
        page==="retornos"    && h(RetornosPendentes,{patients,returnRules,onSelectPatient:handleSelectPatient}),
        page==="agenda"      && h(Agenda,{agenda,setAgenda,procedures:settings.procedures,locations:settings.locations}),
        page==="pacientes"   && h(Patients,{patients,setPatients,onSelect:handleSelectPatient}),
        page==="prontuario"  && !currentPatient && h(Patients,{patients,setPatients,onSelect:handleSelectPatient}),
        page==="prontuario"  && currentPatient  && h(PatientDetail,{patient:currentPatient,setPatients,onBack:()=>setCurrentPatientId(null),procedures:settings.procedures,locations:settings.locations,products:safeList(products).map(p=>p.name),returnRules}),
        page==="estoque"     && h(Estoque,{products,setProducts}),
        page==="financeiro"  && h(Financeiro,{patients,expenses,setExpenses}),
        page==="relatorios"  && h(Relatorios,{patients}),
        page==="mensagens"   && h(MensagensWhatsApp,{}),
        page==="config"      && h(Configuracoes,{settings,setSettings,products,setProducts})
      )
    )
  );
}
